...
We utilized datasets from the Chicago Data Portal, including:
...
2. **American Community Survey (ACS) 5-Year Data by Community Area**: This dataset provides neighborhood-level socioeconomic indicators from the 2019-2023 ACS 5-Year Estimates. Source: U.S. Census Bureau – ACS 5-Year Data (Community Area level).
3. **Community Area Geoboundaries**: Chicago Community Area boundaries from the Chicago Data Portal, used for map-based analysis and dashboard geography.

**Data Retrieval:** Saturday, February 14, 4:17 PM.

### Data Preparation

The raw 311 dataset includes records submitted between January 1, 2023 and December 31, 2024. We filtered observations to keep valid Community Area identifiers and relevant fields (creation date, completion date, request type, service status, and geography).

To measure service efficiency, we constructed response time as completion date minus creation date. Observations with missing completion dates were excluded from response-time calculations.
...
Exclusions and reasons in analysis/dashboard:

- **OHARE**: excluded because it is not directly comparable to residential community areas.
- **311 INFORMATION ONLY CALL**: excluded because it is not an actionable service-delivery category.
- **FOREST GLEN**: excluded from response-time mapping because it behaves as an outlier that compresses map variation for other areas.
...
# Use existing derived outputs for render reproducibility without redownloading raw data.
if not (derived_dir / "df_311_type.csv").exists():
    raise FileNotFoundError("Missing data/derived-data/df_311_type.csv")
if not (derived_dir / "df_311_ca.csv").exists():
    raise FileNotFoundError("Missing data/derived-data/df_311_ca.csv")

# Rebuild static plots quietly from the project script.
subprocess.run(
    [sys.executable, str(code_dir / "plots_static.py")],
    check=True,
    cwd=project_root,
    capture_output=True,
    text=True,
...
This second finding responds to Research Questions 2 and 3. The heatmap shows that demand patterns vary by service category and are not uniform across income groups. Different concentration patterns across cells support the interpretation that neighborhood context matters for both what residents request and how service outcomes differ across issue types.

## Streamlit Dashboard (Brief Explanation)

Interactive app: [https://app-dashboard-qmg6hcjcl7n7vv6yqnpsag.streamlit.app/](https://app-dashboard-qmg6hcjcl7n7vv6yqnpsag.streamlit.app/)

This first graphic shows the relationship between neighborhood income and 311 service outcomes across Community Areas.

We observe both positive and negative trends depending on the service type selected, suggesting that disparities are not uniform across categories.

For some service types, higher-income areas show shorter response times, indicating a potential efficiency advantage. In other cases, the contrary occurs.
...
We used Chicago boundaries from Chicago Data Portal.

## Policy Implications

Chicago should monitor requests per 1,000 residents and average response times by Community Area and income group, rather than relying only on citywide averages. Prioritizing resources where high demand and slower service overlap can improve equity in service delivery.
